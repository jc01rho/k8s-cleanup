...
RUN VERSION="v1.30.0" ; wget https://github.com/kubernetes-sigs/cri-tools/releases/download/$VERSION/crictl-$VERSION-linux-amd64.tar.gz ; tar zxvf crictl-$VERSION-linux-amd64.tar.gz -C /usr/local/bin ; rm -f crictl-$VERSION-linux-amd64.tar.gz
RUN VERSION="1.7.6" ; wget https://github.com/containerd/nerdctl/releases/download/v${VERSION}/nerdctl-${VERSION}-linux-amd64.tar.gz ; tar xzvf nerdctl-$VERSION-linux-amd64.tar.gz -C  /usr/local/bin ; rm -f nerdctl-$VERSION-linux-amd64.tar.gz 
